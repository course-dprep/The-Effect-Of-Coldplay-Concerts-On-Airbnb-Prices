...
Dallas_mean_price_graph <- Dallas_cleaned %>% filter(distance <= 50) %>% 
  ...
  geom_line(color = 'red') + 
  geom_hline(yintercept = mean(Dallas_cleaned$price), color="black")
...
Mexico_distance_price_graph <- ggplot(Mexico_cleaned %>% filter(price < 9999) %>% 
           filter(DuringConcert), mapping = aes(distance, price)) + geom_point(aes(size = price, alpha = 0), colour = 'green') + 
...
Chicago_distance_price_graph <- ggplot(Chicago_cleaned %>% filter(price < 9999) %>% 
           filter(DuringConcert), mapping = aes(distance, price)) + geom_point(aes(size = price, alpha = 0), colour = 'blue') + 
...
ggexport(Chicago_mean_price_graph, filename = '../../gen/output/graph/Chicago/Chicago_mean_price.png')
ggexport(Dallas_mean_price_graph, filename = '../../gen/output/graph/Dallas/Dallas_mean_price.png')
ggexport(Mexico_mean_price_graph, filename = '../../gen/output/graph/Mexico/Mexico_mean_price.png')
ggexport(Chicago_distance_price_graph, filename = '../../gen/output/graph/Chicago/Chicago_distance_price.png')
ggexport(Dallas_distance_price_graph, filename = '../../gen/output/graph/Dallas/Dallas_distance_price.png')
ggexport(Mexico_distance_price_graph, filename = '../../gen/output/graph/Mexico/Mexico_distance_price.png')
ggexport(Chicago_room_type_graph, filename = '../../gen/output/graph/Chicago/Chicago_room_type.png')
ggexport(Dallas_room_type_graph, filename = '../../gen/output/graph/Dallas/Dallas_room_type.png')
ggexport(Mexico_room_type_graph, filename = '../../gen/output/graph/Mexico/Mexico_room_type.png')
